...
//ensure promise chaning for should
...
SaltyWebDriver = _.assign(SaltyWebDriver, {
  ...
  /**
   * Gets usl from the wd config
   * @param url - optional, if null or undefined, it returns the base url only
   * @returns {*}
   */
  getUrl: function (url) {
    if (url == null) {
  ...
  /**
   * returns a configured SaltyWebDriver.js, promiseChainRemote SaltyWebDriver
   * @param config
   * @returns {*}
   */
  //todo: will we consider other SaltyWebDriver libs like SaltyWebDriverio
  ...
  initBrowser: function (browser, config) {
    ...
    //todo:[kj] cache it here (check for mem leaks)
  ...
  inject: function (browser, page, config) {
    ...
    if (!browser)
      return new Error('Must pass instance of SaltyWebDriver browser.');
...
//      $(selector).val(optName);
  ...
  /**
   * returns a SaltyWebDriver promise after picking an item in a select2 within an angular app
   * @param {string} modelKey - data-ng-model attribute of the select2 element
   * @param {string} searchFor - item keyword, will use a css 'contains' call to locate it within a span
   * @param {boolean} [exactMatch] - if false or undefined, it uses contains to locate the word in the span
   * @returns { Q.Promise }
   */
      //todo:[kj] non-ng version require
  select2QuickPicker: function(modelKey, searchFor, exactMatch) {
  ...
  /**
   * returns a promise with the selected text from a select2
  ...
  select2Selected: function (modelKey, searchFor) {
    ...
    //look for a div that is bound to model
  ...
  /**
   * Full featured version of select2Picker.
   * @param {string} modelKey
   * @param {string} searchFor
   * @param {boolean} [searchIsOff]
   * @returns {Promise}
   */
  select2Picker: function (modelKey, searchFor, searchIsOff) {

    if(!searchFor)
      throw new Error('searchFor not defined');
    ...
    stepBrowser = stepBrowser
      ...
      //close it
      // todo: [kj] test option of passing a regex in searchFor instead of string eg. new RegExp('^'+searchString.replace(' ','\s')+'\s\(.*\)$', i) will mean find starts with the word
