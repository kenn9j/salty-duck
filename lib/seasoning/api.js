...
SaltyApiDriver = _.assign(SaltyApiDriver,

    {
      ...
      _hydrateEndpoint: function _hydrateEndpoint(endpointObj, rootObj) {
         ...
         * @private
         */
        function _bindParamsToStringUrlEncoded(templateValueString, endpointItem, rootObj) {

          var bindParams = function (paramVal) {
            if (!paramVal)
              return templateValueString;
            if (typeof(paramVal) == 'string') {
              var boundString = templateValueString;
          ...
          return {
            ...
            getUrl: function (paramVal) {
              var qs = bindParams(paramVal);
              if(!qs.startsWith('?')) qs = '?'+qs;
              return SaltyApiDriver.config.api.apiBaseUrl + rootObj.url + endpointItem.url + qs;
            }
          };
        }
        ...
        _.each(endpointObj, function (endpointItemVal, endpointItemKey) {
          ...
          if (endpointItemKey === 'requests') {
            ...
            _.each(endpointItemVal, function (reqValue, reqKey) {
              ...
              //1.a - GET requests (must contain GetRequest or must be simply 'get')
              if ( /GetRequest/.test(reqKey) || /^get$/i.test(reqKey) ) {
                ...
                if (REGEX_Placeholder.test(reqValObject)) {
                  ...
                  endpointObj['requests'][reqKey] =
                      _bindParamsToStringUrlEncoded(reqValObject, endpointObj, rootObj);
                } else {
                  endpointObj['requests'][reqKey] =
                      _bindParamsToReturnStringAsIs(reqValObject, endpointObj, rootObj);
                }
              }

              //1.b - POST|PUT|DELETE request here
              if (/PostRequest|PutRequest|DeleteRequest/.test(reqKey) || /^(put|post|delete)$/.test(reqKey)) {
