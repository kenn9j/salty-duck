...
SaltyConfig.loadCsvData = function loadData(csvFilePath, options, rowParser) {

  var parse = require('csv').parse;
  var skipHeaders = options ? options.skipHeaders : false;
  console.log(skipHeaders);
  var parser = parse({delimiter: ','},
      rowParser ? rowParser : function (err, data) {
        if (skipHeaders) {
          return Array.slice(data, 0);
          //return;
        }
      });
